...
func _physics_process(delta):
	...
	if (
		level.has_method("_ready")
		and level.get("player_can_move") != null
		and !level.player_can_move
	):
	...
	if direction.length() > 0:
		...
		if (
			animated_sprite.animation.begins_with("Walk")
			or animated_sprite.animation.begins_with("Attack")
		):
